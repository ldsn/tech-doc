...
###接口调用地址
   登陆接口：/wap/login/login
   登陆验证：/wap/login/checkLogin
   退出登陆：/wap/login/logout
###参数说明
...
###请求接口
    用户名验证:/wap/register/checkName   （GET）
    生成验证码:/wap/public/verify（图片获取链接）
    邮箱验证：/wap/register/checkMail （GET）
    重复密码验证：/wap/register/checkPswd  （POST）
###参数说明
####提供参数说明：
<table>
  <tr>
...
##首页，列表
	...
	首页可获取 个人用户信息，文章列表信息，
####请求接口
    获取个人信息：/wap/user/userinfo （GET）
    文章列表信息：/wap/article/getarticle（GET）
    采集文章接口：/wap/article/curlArticle（url形式示例：/wap/article/curlArticle?url=http://blog.wangfengxin.cn/?p=32）
####文章列表参数说明：
---
...
##评论
	评论接口：获取评论，写入评论，删除评论
####请求接口
	获取评论：/wap/comment/getComment?aid=3&count=3
	写入评论：/wap/comment/commentin?uid=4&aid=3&content=3
	删除评论:/wap/comment/commentdelete?com_id=12
####接口参数说明：
